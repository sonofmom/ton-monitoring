...
def run():
    description = "Fetches metrics from TON GLASS Server"
    parser = argparse.ArgumentParser(formatter_class = argparse.RawDescriptionHelpFormatter,
                                    description = description)
    ar.set_standard_args(parser)
    ar.set_config_args(parser)
    ar.set_period_args(parser,default_value=120)
    ...
    rs = gt.send_api_query("{}/{}?period={}".format(cfg.config['glass']['url'], '/overlay_throughput', cfg.args.period))
